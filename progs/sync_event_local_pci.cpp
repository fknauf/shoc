...
auto sync_event_local(doca::progress_engine *engine) -> doca::coro::fiber {
    ...
    if(err != DOCA_SUCCESS) {
        doca::logger->error("failure during cc handshake: {}", doca_error_get_descr(err));
        co_return;
    }
