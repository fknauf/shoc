...
TEST(docapp_eth_frame, ipv4_udp) {
    auto buffer = cppcodec::hex_lower::decode<std::vector<std::uint8_t>>("02d1cf1110511070fdb33a0f0800450000211f9640004011d109c0a86401c0a864dadde53039000d0727663030310a00000000000000000000000000");
    ...
    EXPECT_EQ(segment->source_port(), 0xdde5);
    EXPECT_EQ(segment->destination_port(), 0x3039);
    EXPECT_EQ(segment->length(), 13);
    EXPECT_EQ(segment->checksum(), 0x727);
